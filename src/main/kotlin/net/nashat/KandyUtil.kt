...
fun <T> DataFrame<T>.myPlotMessageCount(
    ...
    val grouppColPath = this.getColumn(groupCol).path()

    return gatheredDf
        .groupBy { grouppColPath and msgCountType }
        .plot {
            area {
...
fun DataFrame<*>.myPlotGroupDeliveredPartsAndMessageTypeCounts(
    adjustX1: Int = 0,
    adjustX2: Int = 0,
    groupCol: ColumnSelector<ResultEntryExploded, *>,
): PlotBunch {

    val df: DataFrame<ResultEntryExploded> = this.cast()

    return plotBunch {
        val minPossibleDisseminationTime =
            df
                .map { 2.0 * config.latencyRounds / config.numberOfChunks }
                .min() + 1.0

        add(
            df.convert(groupCol).toStr()
                .myPlotLinesWithEndingVLine(
                    ...
                    vLine {
                        xIntercept.constant(minPossibleDisseminationTime)
                        type = LineType.DOTTED
                        width = 3.0
                        color = Color.BLACK
                    }
        ...
        add(
            df.myPlotMessageCount(groupCol), x = 0, y = 400, width = 1615, height = 1000
        )
    }
}